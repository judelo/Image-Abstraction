...
void TreeOfShapes::synshapeRect(Shape pShape, Ccimage imgsyn, float *alpha, int *relief, float *reliefOrentation, float *reliefHeight){
...
QImage TreeOfShapes::render(TOSParameters tosParameters, bool &tree_recomputed, QImage image_mask, TreeOfShapes *tosDictionary, DictionaryParameters dictionaryParameters ){
    ...
    Point_plane  ArrayPixelsMask = (Point_plane) malloc(pimage_mask.width() * image_mask.height() * sizeof(struct point_plane));
    Point_plane p;
